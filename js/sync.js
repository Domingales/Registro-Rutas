...
async function pairDevice(){
  ...
  if(navigator.onLine){
    try{
      ...
      await putJSON(devicePath(),{deviceId:getDeviceId(),peerId:peer,lastSeen:nowISO(),routes:getAllRoutes(),accounting:getAllAccounting()});
...
async function syncNow(auto=false){
  ...
  try{
    ...
    const remoteAccounting=remoteDevice?.accounting||[];
    const merged=mergeRoutes(getAllRoutes(), remoteRoutes);
    const mergedAccounting=mergeAccounting(getAllAccounting(), remoteAccounting);
    saveRoutes(merged);
    saveAccounting(mergedAccounting);
    const payload={deviceId:getDeviceId(),peerId:peer,lastSeen:nowISO(),updatedAt:nowISO(),routes:merged,accounting:mergedAccounting};
    ...
    logSync('Sincronización completada. Rutas visibles: '+visibles+'. Registros contables: '+mergedAccounting.filter(e=>!e._deleted).length+'. Total técnico: '+merged.length+'.');
  }catch(e){
