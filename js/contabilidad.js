...
function accountingTotals(rows=getAccounting()){
  ...
  const totalGastos=rows.filter(e=>e.tipo==='gasto'||e.tipo==='gasoil').reduce((s,e)=>s+(parseFloat(e.importeDia)||0),0);
  ...
  return {totalDias,totalGastos,totalACobrar,totalCobrado,saldo};
}
function renderAccounting(){
  ...
  accountingSummary.innerHTML=`
    ...
    <div class="acc-kpi"><span>Total a cobrar / abonar</span><strong>${eur(allTotals.totalACobrar)}</strong></div>
    <div class="acc-kpi"><span>Gastos incluidos</span><strong>${eur(allTotals.totalGastos)}</strong></div>
  ...
  accountingRows.innerHTML=rows.length?rows.map(e=>{
    ...
    return `<tr>
      ...
      <td><span class="tag">${accountingTypeLabel(e.tipo)}</span></td>
...
function accountingTypeLabel(tipo){
  if(tipo==='pago')return 'Pago recibido';
  if(tipo==='gasto')return 'Gasto a abonar';
  if(tipo==='gasoil')return 'Gasoil a abonar';
...
function prepareAccountingForm(tipo='trabajo'){
  ...
  if(tipo==='trabajo'){
    ...
    accConcepto.value='Gasoil a abonar';
    accImportePago.value='';
  }else{
    accConcepto.value='Gasto a abonar';
    accImportePago.value='';
  }
}
